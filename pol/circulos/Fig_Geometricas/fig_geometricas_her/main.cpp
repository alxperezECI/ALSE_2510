...
int main(){
    ...
    Geometrica* pG=0;
    Circulo C1, C2 = Circulo();

    //Bloque del ejercicio de intersección:
    for (int i=0; i<2; i++){
        double radio = 0.;
        double psX = 0.;
        double psY = 0.;
        cout << "Indique el radio del circulo " << i+1 << ": " << endl;
        cin >> radio;
        cout << "Indique la coordenada X del circulo " << i+1 << ": " << endl;
        cin >> psX;
        cout << "Indique la coordenada Y del circulo " << i+1 << ": " << endl;
        cin >> psY;
        if (i == 0){
            C1 = Circulo(radio, psX, psY);
        }else{
            C2 = Circulo(radio, psX, psY);
        }
    }

    if (C1.interseccion(C2) == true){
        cout << "Sí se intersectan los circulos." << endl;
    }else{
        cout << "No se intersectan los circulos." << endl;
    }
    return 0;

    //Comentamos esta sección para dejar ejecutable exclusivamente la sección de intersección del ejercicio.
    /**
    Circulo a = Circulo( 5. );
